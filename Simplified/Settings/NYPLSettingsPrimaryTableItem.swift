...
class NYPLSettingsPrimaryTableItem {
  ...
  class func generateBundledView(title: String, url: URL) -> UIViewController {
    let bundledView = BundledHTMLViewController(fileURL: url, title: title)
    return handleVCWrap(bundledView)
  }
}
